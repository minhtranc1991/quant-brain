...
- **Enrichment — why reference-dependence was necessary at all ([[Thinking, Fast and Slow]]):** Daniel Bernoulli's 1738 expected-utility theory (later formalized by von Neumann-Morgenstern) modeled utility as a function of *final absolute wealth* only, with no concept of a reference point. This cannot explain reference-dependent risk attitudes: two individuals who end up with identical final wealth after a gamble, but who started from different wealth levels, are predicted by Bernoulli's theory to evaluate the outcome identically — empirically false, and unaddressed for over two centuries until prospect theory's reference-dependence assumption. Kahneman and Tversky's own framing states three jointly necessary features of the value function, not loss aversion alone: (1) reference-dependence (correcting Bernoulli's error); (2) diminishing sensitivity applying symmetrically on both sides of the reference point; (3) loss aversion (the loss branch steeper than the gain branch). Source: [[Thinking, Fast and Slow]], Chapters 25-26.
...
- **Enrichment — the fourfold pattern of risk attitudes ([[Thinking, Fast and Slow]]):** the value function's loss aversion alone cannot explain a further, well-documented pattern. Prospect theory's *probability weighting function* systematically overweights small probabilities and underweights moderate-to-high probabilities relative to their true values, producing four distinct risk attitudes depending on the quadrant: risk-averse for high-probability gains (preferring a certain smaller gain over a risky larger one) and for low-probability losses (over-paying for insurance-like protection against an unlikely large loss); risk-seeking for low-probability gains (buying lottery tickets despite negative expected value) and for high-probability losses (rejecting a favorable settlement to gamble on a likely-but-not-certain worse trial outcome). This fourfold pattern is a distinguishing empirical signature of prospect theory versus standard expected-utility theory, which predicts uniform risk aversion. Source: [[Thinking, Fast and Slow]], Chapter 29.
- **Enrichment — broad framing as a risk-policy discipline ([[Thinking, Fast and Slow]]):** loss aversion interacts with decision framing to produce a specific, costly failure mode — "narrow framing" (evaluating each risky decision in isolation) causes rejection of favorable-expected-value risks that would readily be accepted in aggregate, because each individual loss looms too large in isolation. Adopting a deliberate, pre-committed "risk policy" applied consistently across many similar future decisions — evaluated by aggregate expected outcome rather than case-by-case — is prospect theory's own prescriptive antidote to loss-aversion-driven overcaution, directly analogous to portfolio-level versus position-level risk evaluation. Source: [[Thinking, Fast and Slow]], Chapter 31.
- **Enrichment — vividness-dependent rare-event mispricing ([[Thinking, Fast and Slow]]):** the probability weighting function's overweighting of small probabilities applies most strongly to vivid, easily-imagined, emotionally salient rare events; abstract, non-vivid rare events that have not been personally experienced tend instead to be neglected or rounded toward zero. This means the same underlying mechanism can produce either overpricing or underpricing of a given rare event depending on how vividly it is represented — see the nuance added to [[C - Rare Event Risk (Fat-Tail Mispricing)]]. Source: [[Thinking, Fast and Slow]], Chapter 30.
...
- **Enrichment — the theory's own originators' account ([[Thinking, Fast and Slow]]):** Kahneman's first-person account (with Amos Tversky) of developing the theory presents the sure-loss-vs.-gamble reversal and the reference-dependence correction to Bernoulli's expected-utility theory as the discovery's starting point, directly grounding the value-function formalization above in the theory's own origin rather than only in a secondhand (Malkiel) summary. Source: [[Thinking, Fast and Slow]], Chapters 25-26.
