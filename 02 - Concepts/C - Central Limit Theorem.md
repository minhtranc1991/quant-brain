...
- **Enrichment — the "Law of Small Numbers": a cognitive-bias companion to the formal convergence result ([[Thinking, Fast and Slow]]):** the pitfall above is a claim about the formal accuracy of the normal approximation for small samples. Kahneman documents a separate, complementary finding about *intuition*: people — including trained researchers — systematically underestimate how much small samples vary by chance alone, expecting even a small sample to closely resemble the population it was drawn from, and consequently over-interpreting small-sample extremes (in either direction) as meaningful signal rather than expected statistical noise. A documented illustration: county-level kidney-cancer rate rankings show both the *highest* and the *lowest* rates concentrated in the same sparsely-populated (small-sample) counties — the same statistical mechanism (small samples produce more variable statistics) produces both tails, which intuitive, causally-minded analysis tends to explain separately (e.g. attributing low rates to a healthy rural lifestyle, high rates to poverty) rather than recognizing as one shared sampling-variance artifact. This means the CLT's convergence caveat above is not only a technical caution for whoever computes a confidence interval — it also predicts a specific, documented failure of human intuition about small-sample results. Source: [[Thinking, Fast and Slow]], Chapter 10.
