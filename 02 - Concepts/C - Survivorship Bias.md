...
- **Enrichment — mechanism-level account ([[Fooled by Randomness]]):** where the above explains survivorship bias empirically (funds are deliberately closed/merged away), [[Fooled by Randomness]] explains it mechanically, via [[C - Ergodicity]] and initial-population size. A cohort of speculators/funds observed today is a *survived* sample: some fraction of any sufficiently large starting population will show a run of good performance purely by chance, and that fraction is a mechanical function of the population's original size and the number of periods observed — not evidence of skill. By ergodicity, extending the observation horizon further would eventually expose the same "lucky survivors" to the rare, damaging event that already removed the rest of their original cohort; over a sufficiently long horizon they are expected to fare no better than the population's original failures. This reframes the bias as a *sampling-window* artifact (the observer sees only a survived sample path within a fixed window) rather than only a *disclosure* artifact (funds choosing to stop reporting) — the two mechanisms compound rather than compete.
...
- [[C - Ergodicity]] — the mechanism-level explanation (per [[Fooled by Randomness]]) for why any sufficiently large starting population will produce a "surviving" subset that looks skilled by chance alone, given a long enough time horizon and a rare enough adverse event.
- [[C - Alternative Histories]] — survivorship bias is a specific case of ignoring alternative histories: judging a surviving population without considering the failed/discontinued members representing that population's other alternative histories.
