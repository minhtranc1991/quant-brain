...
- **Enrichment — Kahneman's fighter-pilot-instructor illustration, and the causal-misattribution mechanism ([[Thinking, Fast and Slow]]):** Kahneman describes discovering the phenomenon firsthand while teaching Israeli Air Force flight instructors. Instructors observed that cadets praised for an unusually good landing tended to perform worse next time, while cadets criticized after an unusually poor landing tended to perform better next time, and concluded that punishment works and reward backfires. Both patterns are simply the expected statistical signature of regression to the mean for any noisy, imperfectly repeatable skill (an extreme landing reflects both underlying skill and a chance component that does not repeat), independent of any causal effect of the instructor's feedback. This produces a durable, false causal belief precisely because "regression is not a causally interesting fact" — the mind's causal-story-generating machinery (see [[F - System 1 and System 2 (Dual-Process Thinking)]]) fills the explanatory vacuum with the misattributed feedback-consequence story instead of recognizing the statistical artifact. Source: [[Thinking, Fast and Slow]], Chapter 17.
- **Enrichment — a correction procedure for intuitive predictions ([[Thinking, Fast and Slow]]):** because intuitive predictions themselves generally fail to account for regression to the mean (they tend to be as extreme as the evidence they're based on, rather than tempered by how weak that evidence's predictive correlation actually is), Kahneman describes an explicit correction procedure: estimate the correlation $r$ between the predictive evidence and the outcome, then adjust the extreme intuitive prediction only that fraction $r$ of the way from the population baseline toward the intuitive estimate — an actionable, formulaic discipline for de-biasing any forecast (e.g. a manager's or strategy's projected performance from a short, extreme track record) that would otherwise be overconfidently extreme. Source: [[Thinking, Fast and Slow]], Chapter 18.
